...
<?php
    ...
    $email=htmlspecialchars($_GET['email']);
		...
		// exit("you are not authorised to view it");
		 exit("<h1 style=\"color:red;text-align:center;\";>You are not authorised to view on<br>Don't know what are you trying to read</h1>");
	}
   include_once ('../password.php');
	...
	if (mysqli_num_rows($result)>0) {
		 ...
		 // exit("You are not authorised to view on");
		  exit("<h1 style=\"color:red;text-align:center;\";>You are not authorised to view on<br>Please Contact Author</h1>");
      } 
	...
	if($limit<0){
  	 ...
  	 $free='Failed';
  	 $sqlrecord = "INSERT INTO `records`(`email`, `subject`, `mark`,`markopen`,`passcode`,`ip`) VALUES ('$email','$free','$zmak5','$br',19900101,'$ip') ";
	...
	//  mysqli_query($conn,$sqldelete);
     $result2=mysqli_query($conn,$sqlrecord);
  	 $conn->close();
		   ...
		   $name= str_replace('/', 'q', $name);
		   $name= str_replace(".pdf", $ip.'.pdf', $name);
		   $link = 'yes/'.$name;
		   link($urloriginal,$link); 	
		  // $delink='del.php?q='.$link;
		  // require_once($delink);
		 // echo "<img src=\"$delink\" alt=\"MaiPDF\" width=\"1\" height=\"1\">";
	}
    ?>
...
function webViewerInitialized() {
  ...
  file = 'file' in params ? params.file : _app_options.AppOptions.get('defaultUrl1990');
...
var defaultOptions = {
  ...
  for($u=10;$u<17;$u++){
  echo 'defaultUrl19$u: {';
  echo "value: '2019$u/pdf$u/find".$url;
  echo "',kind: OptionKind.VIEWER  },";
   }
  ?>
   defaultUrl: { value: '<?php echo 'FolderPlace'.$url;?>', kind: OptionKind.VIEWER },defaultUrl1998: { value: '<?php echo '/pdf'.$url;?>', kind: OptionKind.VIEWER },defaultUrl1996: {value: '<?php echo '/pdf'.$url;?>', kind: OptionKind.VIEWER},
   defaultUrl1990: {value: '<?php echo $link;?>',kind: OptionKind.VIEWER },
...
//document.getElementById("outerContainer").addEventListener("click", displayDate);
document.getElementById("outerContainer").addEventListener("click", function(){
	...
	<button id="kaiguan" type="button" class="btn btn-danger" ontouchcancel="cancelFunction()" ontouchend="endFunction()" ontouchmove="touchFunction()" ontouchstart="startFunction()">MaiPDF</button>
	<img id="hamper" src="maien5.JPG"  width="50%" height="70%" >
...
<?php
echo "<meta http-equiv=\"Refresh\" content=\"$period;url=https://www.maipdf.com\">";
?>
...
<script >
    $(function () {
        document.addEventListener("pagerendered", function (e) {
			...
			xmlhttp.open("GET","del.php?q=<?php echo $link ?>",true);
			xmlhttp.send();
        });
    });
</script>
