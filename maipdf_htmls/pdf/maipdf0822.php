...
<head>
    ...
    <!-- Main CSS -->
    <link rel="stylesheet" href="maipdf2026006.css?v=<?php echo time(); ?>">
</head>
...
<main>    <!-- Breadcrumb Navigation -->
    ...
    <!-- Settings Section -->    <section class="why-choose-us-section" id="section2">
        <div class="container">
            <form role="form" action="maipdf2026.php" method="post">
                ...
                <div class="settings-grid" id="2step3">
                    ...
                    <div class="setting-box">
                        ...
                        <div id="telegramBindPanel" style="display: none;">
                            ...
                            <div class="form-text mb-3">Add bot → send /start → Get chat_id.</div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', () => {
  const form = document.querySelector('form[role="form"][action="maipdf2026.php"]');
...
<script type="module">
    ...
    // 页面加载时清除旧的 token，确保每次刷新都生成新 token
    sessionStorage.removeItem('telegram_bind_token');

    // 生成或获取 Telegram token
    const generateTelegramToken = async () => {
        try {
            // 每次页面加载都生成新 token，不使用 sessionStorage 中的旧 token
            // 这样可以确保每次刷新页面都有新的 token，避免使用过期或无效的 token

            // 解析 User-Agent，提取简单的浏览器和操作系统信息
    ...
    let statusCheckInterval = null; // 定时器变量
    ...
    // 手动刷新按钮（点击 "Show Telegram chat_id"）
    ...
    // 监听 "Add @maipdfbot" 按钮点击事件
    const bindLink = document.getElementById('telegram-bind-link');
    if (bindLink) {
        bindLink.addEventListener('click', async (e) => {
            // 总是阻止默认行为，确保 token 生成完成后再打开
            e.preventDefault();
