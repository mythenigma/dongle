...
$urljieshu= 'https://maipdf.com/js/sharebylink.html';
...
<?php
  ...
  if(isset ($_GET['email'])){
    ...
    exit( $tuichu);
     ...
     if($email[0]=='d'){
         $url= 'https://maipdf.com/est/?e='.$email;
          header("Location: $url");
          exit();
   }
   // 7.14 
...
$conn = new mysqli($servernameMai, $usernameMai, $passwordMai, $dbnameMai);
  // $conn= new mysqli("127.0.0.1","yesjoe","JOEjoe123","maipdf");
    ...
    $br= $br[0].")";
      $sql2="UPDATE `pdf` SET `limit`=$limit WHERE `mdemail`='$email' ";  
		 if( $limit > -3  && $limit<111000){
		  // 7.14  $result2 = mysqli_query($conn,$sql2);
		  }
  ...
  if($limit<0){
     ...
     // 7.14    $result2=mysqli_query($conn,$sqlrecord);
       $conn->close();
      exit( $tuichu);
  ...
  exit( $tuichu);
     ...
     $free='Succed';
         ...
         if (strcmp($adsensedate, $sqldate) !== 0) {
             if (file_exists($adsensefile)){
               ...
               // $oldadsensedate = substr($oldadsensedate,0,10);
                ...
                $interval=  $interval->days;
			     ...
			     if($interval>90  && !strchr($oldadsensedate,"frequent")){
                     file_put_contents($adsensefile,$adsensedate.'#frequent#'.$interval);
               }
          ...
          $link= str_replace("'","\'",$link);$doctitle= str_replace("'","\'",$doctitle);
   ...
   if(file_exists($verifyfile)){
     ...
     $btofpic= str_replace("'","\'",$btofpic);$bt= str_replace("'","\'",$bt);
   $at=$current[1]; $at=str_replace("'","\'",$at);
  echo "<script>   var emaildizhi='$current[0]'; var at='$at';var bt='$bt'; var btofpic='$btofpic';</script>";
 ...
 <h6  style="text-align:center;"> 
                  <input type="text" value="---" style="color:#404040;text-align:center;background-color: #404040;border-image:url(https://doc.maitube.com/pdf/images/shadow.png);" id="myInput">
                    </h6>
          <h5 id="dibuquanbu" style="text-align:center;color: silver">
          ...
          <h6 style="text-align:center;color: silver">
            <img  id="xiaotu" style="width: 32px;height: auto;" src="https://doc.maitube.club/pdf/images/texture.png"><br>
...
//var mobile = navigator.userAgent.match(/x86_64)!= null;
    if(navigator.userAgent.match(/x86_64/i)!= null){
      ...
      if(url<0){
        $("html").empty();
        window.location.href = "https://maipdf.com/js/sharebylink.html";
      }
    return mobile;
}
            ...
            if(hanren>1){
             ...
             setInterval(swapads, 5999);
              ...
              // setTimeout(function(){ swapno=gracetouch; }, 12000);
               document.getElementById("dibuimg").src = "https://maipdf.com/js/img/grabip.png";
               document.getElementById("dibuad").href='https://grabify.icu?913';
            }
              ...
              if(dubi>30 && verifypdf!=1000  ){
               // verifypdf=1;
                ...
                if(period<2047483647){
                  setTimeout(function(){ window.location.href = "https://maipdf.com/js/sharebylink.html"; }, period);
                }
